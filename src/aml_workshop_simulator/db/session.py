...
def engine_options(config: Settings) -> dict[str, object]:
    options: dict[str, object] = {
        "echo": config.ECHO_SQL,
        "pool_pre_ping": True,
    }
    if config.DB_POOL_DISABLED:
...
async_engine = create_async_engine(settings.database_url, **engine_options(settings))
...
AsyncSessionLocal = async_sessionmaker(
    async_engine, class_=MeasuredSession, expire_on_commit=False
)
