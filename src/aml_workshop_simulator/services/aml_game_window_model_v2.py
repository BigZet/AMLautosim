...
def views(frame):
    result = raw_views(frame)
    # Policy-preserving normalization of inactive evidence. Raw dataset facts
    # remain unchanged. These gates do not compute votes or probabilities.
    active_fanout = (result.fanout >= 2) & (result.recipient_count >= 3)
    active_concentration = (result.sender_count >= 2) & (result.max_recipient_share >= .72)
    result.loc[~(active_fanout | active_concentration), 'precredit_outflow_share'] = 1.
    result.loc[(result.card_count < 7) | (result.second_split_ratio_all > .80), 'amount_repetition_strength'] = 0.
    result.loc[(result.cash < 1) | (result.debit_episodes < 2), 'cash_share'] = 0.
    result.loc[(result.sender_count < 2) | (result.precredit_outflow_share >= .25), 'max_recipient_share'] = 0.
    return result
