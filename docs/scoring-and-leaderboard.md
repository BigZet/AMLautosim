...
и `masked: true`, где `N` — место в общем порядке, а не в текущей странице. Настоящего
ника нет ни в ответе, ни в отрендеренной странице, пока ведущий не нажмет «Показать все
ники» на вкладке «Экран для зала»; тогда админка запрашивает `?reveal=true`, и та же
проекция возвращается с настоящими именами и `masked: false`.

`?reveal=true` требует административной сессии: маскирование защищает проектор, и
снимать его может только тот, кто за проектор отвечает. Участнику этот параметр
недоступен (`403`), анонимному вызову — тоже (`401`).
