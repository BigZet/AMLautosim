...
| Входящий перевод | Источник (банк, криптобиржа, платежный сервис), связь с отправителем |
...
Признаки входящих переводов: `incoming_transfer_sum`, `incoming_transfer_count`,
`crypto_exchange_inflow_sum`, `foreign_bank_inflow_sum`, `primary_incoming_source`,
`primary_sender_relationship`. Входящие переводы не включаются в наличный оборот.
Текущие игровые коэффициенты и параметры описаны в [балансе](../config/BALANCE.md).
