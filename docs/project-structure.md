...
- `catalog.py` — каталог операций и их параметров, включая набор из четырёх операций,
...
Все игровые цены, ограничения, базовый раунд, параметры карточек и коэффициенты
расчёта вынесены в [`config/`](../config/README.md).
Загрузчик — `core/game_config.py`, проверка файлов — `schemas/catalog_config.py`,
фиксация условий раунда — `services/configuration.py`.
