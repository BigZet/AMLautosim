...
### Локальное обновление конструктора

Рабочая область участника выполняется в `st.fragment`: ввод параметров, добавление,
изменение, перестановка и удаление шагов, сохранение, восстановление версии и отправка
обновляют только эту область. Заголовок, навигация и cookie-компонент не запускаются
заново. После явных команд используется `st.rerun(scope="fragment")`; изменения полей
готового шага применяются через `on_change` до расчёта preview, без второго rerun.
Текущий статус раунда по-прежнему читается при каждом запуске рабочей области.

Энергия, время и доступные шаги показываются как «остаток из начального значения»;
начальные значения берутся из конфигурации текущего раунда. Баланс остаётся суммой.
При отсутствии нарушений сообщение не выводится. При наличии показывается компактная
свёрнутая строка с количеством нарушений и раскрываемыми подробностями: под новой
операцией для preview или перед сохранением для текущей цепочки. Проверка остаётся
мгновенной, а серверные проверки сохранения и отправки не меняются.

Основание: [фрагменты Streamlit](https://docs.streamlit.io/develop/concepts/architecture/fragments)
и [область действия rerun](https://docs.streamlit.io/develop/api-reference/execution-flow/st.rerun).
