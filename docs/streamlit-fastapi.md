...
Streamlit отдает одну серверную тему на все браузеры, поэтому светлый режим реализован
как per-browser overlay: приложение переопределяет CSS-переменные и явно перекрашивает
виджеты, у которых цвет пришел из серверной темы. Выбор хранится в cookie `aml_theme`,
темная тема — стартовая, переключатель есть на экранах входа и внутри обеих панелей.

Внутри одного сеанса выбор пользователя всегда важнее прочитанного cookie: cookie
дочитывается только при инициализации, иначе гонка между записью cookie и следующим
rerun возвращала бы предыдущую тему.
