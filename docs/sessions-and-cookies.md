...
## 9.2. Cookie оформления

Выбор темы хранится в отдельном cookie `aml_theme` (`dark` по умолчанию). Он не
относится к аутентификации, не содержит персональных данных и живет рядом с session
cookie: тема переживает навигацию и перезагрузку, но не подменяет собой серверное
состояние. Внутри одной вкладки выбор пользователя всегда важнее прочитанного cookie,
иначе гонка между записью cookie и следующим rerun возвращала бы старую тему.
