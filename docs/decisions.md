...
> Обновление ядра: актуальная модель одной игры описана в [architecture.md](architecture.md), новый API — в [api.md](api.md). Streamlit и тесты пока относятся к прежней модели; план адаптации — [streamlit-next-step.md](streamlit-next-step.md). Упоминания истории, шаблонов, остановки и ручных оценок ниже устарели.
...
**Решение.** Каталог и конструктор содержат ровно четыре операции: `salary`,
`incoming_transfer`, `card_transfer`, `cash_withdrawal`. У каждой не более двух
настраиваемых параметров, включая канал. Остальные параметры сервер закрепляет
значениями по умолчанию. Блок `operations` в снимке раунда может только отключить
часть этого набора, но не добавить другой код.

**Причины.** Универсальная матрица контекста перегружала интерфейс и делала выбор
неинформативным. Текущая цель 400 000 ₽ требует цепочки операций; источники входящего перевода
и связь с отправителем дают два отдельных выбора. Наличные ограничивают только снятия.

**Последствия.** Скрытые параметры обязаны иметь стабильные серверные значения, иначе
скоринг перестанет быть детерминированным. Клиент, приславший скрытый или чужой параметр, получает `422`
...
## D26. Темой управляет Streamlit

**Решение.** В приложении нет своих кнопок смены темы. Зритель выбирает
System, Light или Dark в верхней части штатного меню Streamlit `⋮`. Для светлого и
тёмного режимов заданы отдельные палитры в `.streamlit/config.toml`; базовая тема не
фиксируется, поэтому System следует настройке операционной системы.

**Причины.** Штатный механизм перекрашивает все виджеты согласованно, хранит выбор
на стороне браузера и не требует cookie, `st.session_state`, rerun или CSS-overlay.
Кастомные стили ссылаются на переменные Streamlit и поэтому следуют его активной теме.

**Пересмотреть, если:** штатное меню тем Streamlit будет отключено или убрано.
...
## D28. Входящий перевод вместо внесения наличных

Новая карточка `incoming_transfer v1` заменяет внесение в живом каталоге.
Источник и связь с отправителем — параметры действия; канал — банковское зачисление.
Криптобиржа моделирует рублевое зачисление, банк Кыргызстана — учебный международный
перевод. Настройки риска не являются рейтингами стран или реальных организаций.
Старые раунды используют сохраненные контракты `cash_deposit`; seed не переписывает
их снимки. Новая игра принимает только новый код и новые поля.
