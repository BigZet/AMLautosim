...
from scripts.aml_game_curriculum_v4 import candidate, catalog
...
def family_counts(total):
    ...
    return {f'{band}:{start // 5000}': min(5000, count-start)
            for band, count in totals.items() for start in range(0,count,5000)}
