...
def run(package):
    ...
    def score(steps):
        ...
        x = pd.DataFrame([features])[manifest['features']]
        margin = float(model.predict(x, prediction_type='RawFormulaVal')[0])
        cal = manifest['calibration']
        z = cal['a'] * margin + cal['b'] if cal['method'] == 'sigmoid' else margin
