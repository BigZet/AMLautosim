...
def train(dataset, output):
    ...
    # Balance route coverage during fitting only. Both class contributions of
    # each row receive the same factor, preserving its target vote fraction.
    train_rows = parts['train']
    counts = train_rows.route_family.value_counts()
    factors = (len(train_rows) / len(counts) / train_rows.route_family.map(counts)).to_numpy()
    target_train = train_rows.target_probability.to_numpy()
    weight *= np.r_[factors, factors][np.r_[target_train, 1-target_train] > 0]
    ...
    manifest = dict(
        ...
        training_weighting='Equal total contribution per route family; calibration and evaluation retain natural population weights',
