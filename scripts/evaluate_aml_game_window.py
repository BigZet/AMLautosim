...
def run(package):
    ...
    from src.aml_workshop_simulator.services.aml_game_window_model import score_features
    require(manifest['architecture'] == 'mean_of_three_shared_classifier_views', 'Wrong architecture')
    require(manifest['inference_source_sha256'] == file_hash('src/aml_workshop_simulator/services/aml_game_window_model.py'), 'Inference source drift')
    def score(steps):
    ...
    cases = []
    for seed in [2026200216, 2026200696, 2026201128, 2026200816, 2026200936] + list(range(2026300000, 2026302400)):
        ...
        if len(cards) >= 2:
            cards[0]['amount'] = str(float(cards[0]['amount']) + .01)
            cards[1]['amount'] = str(float(cards[1]['amount']) - .01)
    ...
    report = dict(counts=dict(counts), low_families=sorted(low_families),
