...
from src.aml_workshop_simulator.domain.channels import ALL_CHANNELS, Channel
...
def test_global_channel_enum_is_exactly_the_seven_known_values() -> None:
...
def test_pos_is_a_known_channel_that_no_card_allows() -> None:
    """A UI label alone must never make a channel usable."""
    assert Channel.pos in set(Channel)
    for channels in EXPECTED_MATRIX.values():
        assert "pos" not in channels
...
def test_declared_channel_is_accepted(spec_by_code, specs, game_config, code, channel) -> None:
    ...
    if spec.requires_card_code:
        ...
        steps.append(make_step(prerequisite, prerequisite.max_amount))
    steps.append(make_step(spec, spec.min_amount, channel=channel))
