...
from local_store import ACTION_CARDS, resource_snapshot
...
def render_sidebar(rounds: list[dict]) -> dict | None:
    with st.sidebar:
        ...
        mode = "Интерактивный прототип"
        endpoint = "Данные текущей Streamlit-сессии"
        st.markdown(
            f"""
        ...
        with st.expander("Создать раунд"):
            st.caption("Создание раундов не входит в автономный UI-прототип.")
...
def render_command_bar(round_data: dict) -> None:
    ...
    if refresh_col.button(
        ...
        st.rerun()
    st.caption("Команды раунда показаны как референс и отключены в UI-прототипе.")
...
def render_admin() -> None:
    ...
    st.markdown(
        ...
        f"<span>Источник: демо-данные Streamlit-сессии</span>"
