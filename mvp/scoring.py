from action_parameters import action_detail_effects
from enums import RiskLabel


COUNTRY_RISK_POINTS = {"low": 0, "medium": 10, "high": 24}
RECIPIENT_POINTS = {"known_counterparty": 0, "new_counterparty": 12, "anonymous_wallet": 25}
TIME_OF_DAY_POINTS = {"day": 0, "evening": 3, "night": 9}
VELOCITY_POINTS = {"spaced": -4, "normal": 0, "rapid": 12}
CHANNEL_POINTS = {"bank": -2, "branch": -3, "mobile": 0, "web": 2, "atm": 3, "exchange": 6}


def score_steps(steps: list[dict], card_weights: dict[str, float]) -> tuple[float, RiskLabel, dict]:
    factors: list[dict] = []
    total = 0.0
    ...
    for index, step in enumerate(steps, start=1):
        ...
        channel = step.get("channel", "mobile")
        has_documents = bool(step.get("has_documents", True))
        detail_effects = action_detail_effects(card_code, step.get("details"))

        step_factors = [
            _factor(
                index,
                f"card:{card_code}",
                float(card_weights.get(card_code, 0)),
    ...
    explanation = {
        ...
        "note": "Учебная модель для обсуждения, а не реальная банковская AML-система.",
    }
...
def _sequence_factors(steps: list[dict]) -> list[dict]:
    factors: list[dict] = []
    ...
    for index in range(1, len(steps)):
        ...
        current = steps[index]
        previous_gross = float(previous.get("amount", 0)) * int(previous.get("frequency", 1))
        current_gross = float(current.get("amount", 0)) * int(current.get("frequency", 1))
        if (
            previous.get("card_code") in credit_codes
            and current.get("card_code") in debit_codes
    ...
    for index, step in enumerate(steps):
        if step.get("card_code") != "cash_deposit":
            continue
        following = steps[index + 1 : index + 3]
        if any(item.get("card_code") in {"crypto_exchange", "international"} for item in following):
            factors.append(
                _factor(
                    index + 2,
...
def _factor(step: int, name: str, points: float, description: str) -> dict:
    return {
        "step": step,
        "name": name,
